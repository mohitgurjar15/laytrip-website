...
export class ShareSocialMediaComponent implements OnInit {
  ...
  share(media){
    if(media == 'Pinterest'){
      ...
      window.open(`https://pinterest.com/pin/create/bookmarklet/?url=${environment.siteUrl}`);
      ...
      window.open(`whatsapp://send?text= Share via WhatsApp , See More: ${environment.siteUrl}`);      
    } 
    return false;
