...
export class FlightSearchWidgetComponent implements OnInit {
  ...
  typeOf(value) {
    //console.error("valuewwwwwwwwwwwwwwww",value)
    return typeof value;
  }
  ngOnInit(): void {
    this.fromSearch = [];
    ...
    this.homeService.getToString.subscribe(toSearchString => {
      if (typeof toSearchString != 'undefined' && Object.keys(toSearchString).length > 0) {
        ...
        this.searchFlightInfo.departure = this.fromSearch.code;
        if(toSearchString && toSearchString.to && toSearchString.to.code){
          this.toSearch = airports[keys.to.code];
        }else{
          this.toSearch = airports[keys.code]
        }
        this.homeService.getLandingPageData.subscribe(data => {
          try {
  ...
  dealsToggle() {
    ...
    if (this.dealIcon === true) {
      ...
      this.flightSearchForm.controls.toDestination.setValue(null)
      this.fromSearch = {}
      this.toSearch = {}
  ...
  searchFlights() {
    ...
    queryParams.dealsIcon = this.dealIcon;
    if (this.commonFunction.isRefferal()) {
